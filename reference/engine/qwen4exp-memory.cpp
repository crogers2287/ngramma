...
void llama_model_qwen4exp::load_arch_tensors(llama_model_loader & ml) {
    if (auto overlay = flash_memory::overlay::read()) {
        if (!params.ple_on_disk || params.path_ple) throw std::runtime_error("FML requires --ngram-on-disk on the original joined table");
        if (params.kv_overrides && params.kv_overrides[0].key[0]) throw std::runtime_error("FML refuses metadata overrides on a pinned model");
        overlay->verify_model(ml.fnames);
    }
    LLAMA_LOAD_LOCALS;
...
ggml_tensor * llama_model_qwen4exp::graph::build_attn_qsa(
    ...
    static const bool gather_enabled = [] {
        const char * value = getenv("QWEN4EXP_QSA_GATHER");
        return value == nullptr || atoi(value) != 0;
    }();
    ggml_tensor * cur = gather_enabled && cparams.flash_attn && 4*n_tps*width < n_kv
...
void llm_graph_input_ple::set_input(const llama_ubatch * ubatch) {
    ...
    if (const char * path = getenv("FLASH_MEMORY_TRACE")) {
        std::ofstream trace(path, std::ios::app);
        if (!trace) throw std::runtime_error("FML cannot open address trace");
        for (int64_t i = 0; i < n_tokens; ++i) {
            flash_memory::json record = {{"position", ubatch->pos[i]}, {"sequence", ubatch->seq_id[i][0]}, {"token", tok_of(i)}};
            record["rows"] = std::vector<int32_t>(idx.begin() + i*n_heads, idx.begin() + (i+1)*n_heads);
            trace << record.dump() << "\n";
        }
    }
