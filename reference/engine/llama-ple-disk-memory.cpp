...
struct llama_ple_disk::impl {
    std::unique_ptr<flash_memory::overlay> memory_overlay;
    std::string fname;
    int         fd     = -1;
    bool        direct = false;
    size_t      offs   = 0;
    ...
    impl(const std::string & fname, size_t offs, ggml_type type, int64_t ne0, int64_t nrows, const params & p)
        ...
        memory_overlay = flash_memory::overlay::read();
        if (memory_overlay) memory_overlay->verify_table(fname, offs, type, ne0, nrows);
    ...
    void gather(const int32_t * idx, size_t n, float * dst) {
        ...
        for (size_t k = 0; k < n; ++k) {
            ...
            const float * replacement = memory_overlay ? memory_overlay->find(idx[k]) : nullptr;
            if (replacement) {
                memcpy(out, replacement, size_t(ne0) * sizeof(float));
            } else if (to_float) {
