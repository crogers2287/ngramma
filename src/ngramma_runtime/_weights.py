...
from .environment import worker_imports, config, RuntimeDependencyError
worker_imports()
import ctypes
import time
import weakref
...
class EngineWeights(GgufWeights):
    def __init__(self, paths, ram_cache_bytes=1 << 30):
        bridge = config().validate().bridge_path
        try:
            self.lib = ctypes.CDLL(str(bridge))
        except OSError as exc:
            raise RuntimeDependencyError(f"Cannot load matching dequantization bridge {bridge}: {exc}") from exc
        if not hasattr(self.lib, "flash_memory_dequant"):
            raise RuntimeDependencyError(f"Bridge {bridge} lacks flash_memory_dequant; rebuild the matching research bridge")
