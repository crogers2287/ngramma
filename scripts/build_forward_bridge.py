#!/usr/bin/env python3
"""Compile a local forward-only GGML bridge; never build/start an engine.
...
def main():
    ...
    source = Path(__file__).resolve().parents[1] / 'src/ngramma_runtime/native/ggml_forward.cpp'
    ...
    with tempfile.TemporaryDirectory(prefix='.ngramma-forward-', dir=output.parent) as temporary:
        ...
        replacements = {str(include): '<ENGINE_SOURCE>/ggml/include',
                        str(source): '<REPOSITORY>/src/ngramma_runtime/native/ggml_forward.cpp',
        ...
        record = {
            ...
            'sources': {'ggml_forward.cpp': sha256(source),
                        'build_forward_bridge.py': sha256(Path(__file__).resolve())},
            ...
            'qualification': 'Compiled forward-only primitive bridge; no model-parity or gradient qualification',
        }
    ...
    print(f'Built forward-only diagnostic bridge: {output}')
